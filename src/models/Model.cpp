...
vector<TextureData> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, const std::string &typeName) {
    vector<TextureData> textures;
...
unsigned int loadTextureFromFile(const char *path, const string &directory, bool gamma) {
    ...
    if (data) {
        ...
        if (nrComponents == 1)
            ...
            throw std::runtime_error("Unknown number of image components: " + std::to_string(nrComponents));
        }
